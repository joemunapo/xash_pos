<template>
  <UserLayout>
    <div class="space-y-6">
      <!-- Welcome Section -->
      <div class="bg-gradient-to-r from-brand-600 to-brand-600 dark:from-brand-500 dark:to-brand-500 rounded-xl shadow-lg p-6 sm:p-8 text-white">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-3xl sm:text-4xl font-bold">Welcome back, {{ user.name }}!</h1>
            <p class="text-brand-50 mt-2">Member since {{ user.created_at }}</p>
          </div>
        ...
        <div class="lg:col-span-1">
          <div class="bg-white dark:bg-slate-800 rounded-lg shadow p-6 border border-gray-200 dark:border-slate-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
              <i class="fas fa-lightning-bolt text-brand-600 dark:text-brand-400"></i>
              Quick Actions
            </h2>
            <div class="space-y-2">
              <button class="w-full px-4 py-2.5 bg-brand-50 dark:bg-brand-900/20 text-brand-700 dark:text-brand-300 rounded-lg font-medium hover:bg-brand-100 dark:hover:bg-brand-900/40 transition-colors text-sm">
              ...
              <button class="w-full px-4 py-2.5 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded-lg font-medium hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors text-sm">
                <i class="fas fa-box mr-2"></i>New Order
              </button>
          ...
          <div class="bg-white dark:bg-slate-800 rounded-lg shadow p-6 border border-gray-200 dark:border-slate-700">
            ...
            <div v-if="recent_activity.length > 0" class="space-y-4">
              <div v-for="activity in recent_activity" :key="activity.id" class="flex items-start gap-4 p-3 rounded-lg bg-gray-50 dark:bg-slate-700/50">
                <i :class="`fas ${activity.icon} text-brand-600 dark:text-brand-400 mt-1`"></i>
            ...
            <div v-else class="text-center py-8">
              ...
              <p class="text-sm text-gray-500 dark:text-gray-500 mt-1">Start by creating a new task or order</p>
            </div>
          </div>
        </div>
      </div>
