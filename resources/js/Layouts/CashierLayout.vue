      ...
      <div class="h-16 flex items-center justify-between px-4 border-b border-gray-100 dark:border-slate-800 flex-shrink-0 bg-gradient-to-r from-gray-50/50 to-transparent dark:from-slate-800/50">
        <div :class="['flex items-center flex-1', miniSidebar ? 'justify-center' : 'gap-2']">
          <Link :href="route('cashier.dashboard')" :class="['flex items-center group', miniSidebar ? '' : 'gap-2.5']">
            <div class="w-10 h-10 bg-gradient-to-br from-brand-500 via-brand-600 to-cyan-600 rounded-2xl flex items-center justify-center shadow-lg shadow-brand-500/30 flex-shrink-0 group-hover:scale-105 transition-transform">
              <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path>
              </svg>
            </div>
            <span v-if="!miniSidebar" class="text-lg font-bold bg-gradient-to-r from-gray-900 to-gray-700 dark:from-white dark:to-gray-300 bg-clip-text text-transparent whitespace-nowrap">XASH<span class="bg-gradient-to-r from-brand-500 to-cyan-500 bg-clip-text">POS</span></span>
          </Link>
        </div>
      ...
      <div class="border-t border-gray-100 dark:border-slate-800 p-3 bg-gradient-to-r from-gray-50/50 to-transparent dark:from-slate-800/50 relative flex-shrink-0">
        ...
        >
          <div :class="['flex items-center', miniSidebar ? '' : 'gap-3']">
            <div class="w-10 h-10 rounded-full bg-gradient-to-br from-brand-400 via-brand-500 to-cyan-500 flex items-center justify-center text-white text-sm font-bold shadow-lg ring-2 ring-white dark:ring-slate-900">
